    ...
    try {
      ...
      let { text: response, attachments } = await this.spaceManager.sendMessageToMemberIfNotPaused(
        ...
        (cumulativeContent) => {
          ...
          // Notify HubClient to forward stream update
          this.onStreamMessage?.(member, fullContent);
        },
      ...
      if (!attachments || attachments.length === 0) {
        ...
        await this.sendMessage(member, response, attachments);
...
请只输出 JSON，不要其他内容。`;
  ...
  private async sendMessage(member: Member, content: string, attachments?: Omit<Attachment, 'id' | 'messageId' | 'createdAt'>[]): Promise<Message> {
    ...
    try {
      ...
      if (this.onMessageSent) {
        logger.info(`[AIController] Calling onMessageSent callback`);
        this.onMessageSent(member, content);
      } else {
        logger.warn(`[AIController] onMessageSent callback not set!`);
      }
      return message;
    } catch (err) {
  ...
  /**
   * 流式消息回调（由 HubClient 设置）
   * 用于实时更新消息内容到前端
   */
  onStreamMessage?: (member: Member, content: string) => void;
...
请回应（简短，1-2句话）：`;
          ...
          const { text: response, attachments } = await this.spaceManager.sendMessageToMemberIfNotPaused(
            ...
            (cumulativeContent) => {
              ...
              this.onStreamMessage?.(responder, fullContent);
            },
            message.id // Pass messageId for tool status tracking
          );
      ...
      try {
        ...
        const { text: response, attachments } = await this.spaceManager.sendMessageToMemberIfNotPaused(
          ...
          (cumulativeContent) => {
            ...
            this.onStreamMessage?.(starter, fullContent);
          },
          message.id // Pass messageId for tool status tracking
        );
